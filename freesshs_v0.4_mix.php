...
// SIMPLE LOGIN SYSTEM
	If(isset($_POST['username']) && isset($_POST['password'])) {
		...
		echo '<textarea rows="8" class="input-sm form-control" disabled readonly style="resize: none; width: 325px; text-align: center; cursor: default;">'.file_get_contents($user."_ip.log").'</textarea>';
		echo '</div>';
		...
		echo '<p>Scan logs:</p>';
		echo '<div align="center" class="form-group">';
		echo '<textarea class="input-sm form-control" rows="8" disabled readonly style="resize: none; width: 325px; text-align: center; cursor: default;">'.file_get_contents("scan_logs.log").'</textarea>';
		echo '</div>';
		echo '<form action="" method="post">';
		echo 'Clear: ';
...
<center><h1>ROOT-URI FREE v0.4</h1>
<p><?php
...
echo "Nu am prins nimic! Inca incerc sa prind... Apasa F5 sa vezi daca am prins ceva!";
	...
	echo '<textarea rows="5" class="input-sm form-control" readonly style="resize: none; width: 500px; text-align: center; cursor: default;">'.$data.'</textarea>';
}
?></p></center>
<audio autoplay="" loop="" id="music" preload="auto" src="/music.mp3"></audio><br>
<center>
<button id="mute" class="btn btn-sm btn-default" type="button">Mute/Unmute Music</button>
